    ...
    public class ProductController : Controller
    {
        ...
        public ActionResult Add(ProductViewModel productViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                var isCodeExist = _productManager.IsCodeExist(productViewModel.Code);
                if (isCodeExist)
                {
                    productViewModel.CategorySelectListItems = _categoryManager.GetAll().Select(c => new SelectListItem()
                    {
                    ...
                    return this.View("Add", productViewModel);
                }

                // Name is already Exist
                var isNameExist = _productManager.IsNameExist(productViewModel.Name);
                if (isNameExist)
                {
                    productViewModel.CategorySelectListItems = _categoryManager.GetAll().Select(c => new SelectListItem()
                    {
        ...
        public ActionResult Edit(ProductViewModel productViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (isCodeExist)
                {
                    ...
                    ModelState.AddModelError("CodeExist", "Code already exist");
                    return this.View("Edit", productViewModel);
                }
